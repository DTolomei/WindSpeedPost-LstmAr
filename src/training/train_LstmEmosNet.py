...
def LstmEmosNet(hp):
    ...
    # Output layers
    out = tfkl.LSTM(2, activation='softplus', return_sequences=True)(x)
...
with open(home+'res/LstmEmosNetHp.pkl','rb') as f:
    hp = pkl.load(f)

print('Train LstmEmosNet')
for i in range(2):
    model = LstmEmosNet(hp)
    
    hist = model.fit(
            train_dataset,
            epochs=150,
            callbacks=[
                    tf.keras.callbacks.ModelCheckpoint(home + f'res/ckpts/LstmEmosNetTrial{i}.h5', monitor='loss', save_best_only=True),
                ],
            )
